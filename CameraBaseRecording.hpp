...
/**
 * @brief CameraBase 生产者侧 JPEG 图像内录和断电恢复。
 *
...
class CameraBaseRecording
{
 public:
  ...
  void Open(std::string_view camera_name, const CameraBaseRecordingParam& param)
  {
    ...
    enabled_ = true;
    XR_LOG_PASS(
        "CameraBase(%s): recording enabled temp=%s final=%s stem=%s codec=jpeg quality=%d source_bytes_per_frame=%u",
        name_.c_str(), output_dir_.c_str(), final_output_dir_.c_str(),
        file_stem_.c_str(), jpeg_quality, static_cast<unsigned>(image_bytes));
  }
  ...
  bool Record(const std::array<uint8_t, image_bytes>& data, uint64_t timestamp_us)
  {
    ...
    frames_.write(reinterpret_cast<const char*>(encoded.data()),
 ...
 private:
  /**
   * @brief 将当前图像编码为 JPEG blob。
   */
  static bool EncodeJpeg(const std::array<uint8_t, image_bytes>& data,
                         std::vector<uint8_t>& encoded)
  {
    ...
    else
    {
      ...
      return cv::imencode(".jpg", image, encoded, params) && !encoded.empty();
    }
  }
  ...
  void WriteCameraInfo(const std::filesystem::path& path) const
  {
    ...
    out << "frame_codec: jpeg\n";
    out << "jpeg_quality: " << jpeg_quality << "\n";
  ...
  /**
   * @brief 修复 JPEG blob 文件和帧索引 CSV 的断电尾部。
   */
  ...
  std::ofstream frames_{};  ///< JPEG blob 输出流。
