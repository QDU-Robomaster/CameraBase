...
class CameraBaseRecording
{
 public:
  ...
  void Open(std::string_view camera_name, const CameraBaseRecordingParam& param)
  {
    ...
    if (!ParseRecordingCodec(param.codec, recording_codec_))
    {
      XR_LOG_ERROR("CameraBase(%s): unsupported recording codec '%s'",
                   name_.c_str(), std::string(param.codec).c_str());
      throw std::runtime_error("CameraBase: unsupported recording codec");
    }
    if (recording_codec_ == RecordingCodec::PNG && !PngSupported())
    {
    ...
    recording_png_compression_ =
        param.png_compression > 9U ? 9U : param.png_compression;
    ...
    csv_ << "frame_index,camera_timestamp_us,offset_bytes,size_bytes,codec\n";
    enabled_ = true;
    XR_LOG_PASS(
        "CameraBase(%s): recording enabled temp=%s final=%s stem=%s codec=%s png_compression=%u source_bytes_per_frame=%u",
        name_.c_str(), output_dir_.c_str(), final_output_dir_.c_str(),
        file_stem_.c_str(), recording_codec_name_.c_str(),
        static_cast<unsigned>(recording_png_compression_),
  ...
  /**
   * @brief 写入一帧图像载荷和对应帧索引 CSV。
   *
   * @param data 未压缩图像字节，布局必须与 `CameraInfoV` 一致。
  ...
  bool Record(const std::array<uint8_t, image_bytes>& data, uint64_t timestamp_us)
  {
    ...
    {
      if (!EncodePng(data, encoded_frame_))
      {
        XR_LOG_ERROR("CameraBase(%s): png encode failed at frame=%u",
                     name_.c_str(), static_cast<unsigned>(frame_index_));
        return false;
      }
      payload = encoded_frame_.data();
      payload_size = encoded_frame_.size();
    }

    frames_.write(reinterpret_cast<const char*>(payload),
 ...
 private:
  ...
  /**
   * @brief 返回 CSV / YAML 中使用的 codec 名称。
   */
  ...
  /**
   * @brief 当前源图像编码是否可直接写成 PNG。
   */
  static constexpr bool PngSupported()
  {
  ...
  /**
   * @brief 将源字节包装成 OpenCV 图像视图。
   */
  static cv::Mat MakeImageMat(const std::array<uint8_t, image_bytes>& data)
  {
  ...
  /**
   * @brief 将当前图像编码为 PNG blob。
   */
  bool EncodePng(const std::array<uint8_t, image_bytes>& data,
                 std::vector<uint8_t>& encoded) const
  {
    const cv::Mat image = MakeImageMat(data);
    if (image.empty())
    {
      return false;
    }
    const std::vector<int> params{cv::IMWRITE_PNG_COMPRESSION,
                                  static_cast<int>(recording_png_compression_)};
    return cv::imencode(".png", image, encoded, params) && !encoded.empty();
  }
  ...
  /**
   * @brief 解析帧索引 CSV 的一行，返回 blob 偏移和大小。
   */
  static bool ParseFrameCsvRow(const std::string& line, uint64_t& offset, uint64_t& size)
  {
    ...
    {
      if (!ParseUint64Token(tokens[i], values[i]))
      {
        return false;
      }
    }
    if (!tokens[4].empty() && Trim(tokens[4]).empty())
    {
      return false;
    }
    offset = values[2];
  ...
  /**
   * @brief 修复图像 blob 文件和帧索引 CSV 的断电尾部。
   */
  static void RepairFrameFiles(const std::filesystem::path& dir, std::string_view stem)
  {
    ...
    const auto blob_file_size = std::filesystem::file_size(frames_path, ec);
    if (ec)
    {
      return;
    }
    ...
    while (std::getline(input, line))
    {
      ...
      if (size == 0 || offset > blob_file_size || size > blob_file_size - offset)
      {
        break;
      }

      repaired << line << "\n";
      valid_blob_end = offset + size;
    }

    std::ofstream output(csv_path, std::ios::out | std::ios::trunc);
    output << repaired.str();
    output.flush();
    if (valid_blob_end < blob_file_size)
    {
      std::filesystem::resize_file(frames_path, valid_blob_end, ec);
    }
  }
  ...
  RecordingCodec recording_codec_{RecordingCodec::PNG};  ///< 当前图像载荷编码。
  std::string recording_codec_name_{"png"};  ///< CSV / YAML 中写出的载荷编码名。
  uint8_t recording_png_compression_{1};  ///< 当前 PNG 压缩级别。
  ...
  std::ofstream frames_{};  ///< 图像 blob 输出流。
  std::ofstream csv_{};  ///< 图像帧索引 CSV 输出流。
  std::vector<uint8_t> encoded_frame_{};  ///< 复用的压缩输出缓冲。
};
