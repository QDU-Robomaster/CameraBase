...
- `frames.bin`：所有图像原始字节顺序追加，像素格式与 `CameraInfo.encoding` 一致。
- `frames.csv`：每帧的 `frame_index,camera_timestamp_us,offset_bytes,size_bytes`。
- `camera_info.yaml`：宽、高、step、encoding、单帧字节数和相机名。
